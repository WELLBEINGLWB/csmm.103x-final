...
class MoveArm(object):
    ...
    def is_valid(self,q_start,q_goal):
        step = 0.2
        dist = q_goal - q_start
        normd = numpy.linalg.norm(dist)
        unitd = dist/normd
        disc_points = numpy.outer(numpy.arange(step,normd,step),unitd) + q_start
        prev = [q_start]
        for disc_point in disc_points:
            if self.is_state_valid(disc_point) == False:
                return prev
            prev = numpy.array([disc_point])
        return q_goal
    ...
    def motion_plan(self, q_start, q_goal, q_min, q_max):
        ...
        step = 0.5
        while True:
            if numpy.allclose(self.is_valid(q_space[-1],q_goal),numpy.array([q_goal])):
                near = q_space[-1]
            ...
            for i in range(self.num_joints):                
                qrand.append(random.uniform(-3.1459,3.1459))
            qrandn = numpy.array([qrand])
            vtpoint = q_space - qrandn
            nearind = numpy.argmin(numpy.sqrt(numpy.sum((vtpoint)**2,axis=1)))
            near = q_space[nearind]
            nearran = qrandn - near
            normn = numpy.linalg.norm(nearran)
            unitn = nearran/normn
            next = near + unitn*step
            valid = self.is_valid(near,next)
            q_space = numpy.append(q_space,valid,axis=0)
            q_index.append(nearind)
        q_list = []
        curind = q_index[-1]
        q_list.append(q_goal)
        for i in range(len(q_space)):
            if  numpy.all(q_space[curind] == q_start):        
                q_list.append(q_start.tolist())
                break
            q_list.append(q_space[curind].tolist())
            curind = q_index[curind - 1]
        q_list.reverse()
        return q_list 
