...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    current_T_des = numpy.dot(ee_current_T_b, b_T_ee_desired)
    btee = tf.transformations.translation_from_matrix(current_T_des)
    bRee = current_T_des[:3,:3]
    angle, axis = rotation_from_matrix(current_T_des)
    ROT = numpy.dot(angle,axis)
    eeRb = tf.transformations.inverse_matrix(bRee)
    tra_gain = 3.5
    rot_gain = 1.7

    del_X = numpy.append(btee * tra_gain, ROT * rot_gain)
